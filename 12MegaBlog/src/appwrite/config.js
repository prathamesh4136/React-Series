...
export class Service {
    ...
    async createPost({title, slug, content, featuredImage, status, userId}) {
        try {
            if (!slug) throw new Error("Slug is required and must be unique!");
    
            return await this.databases.createDocument(
                conf.appwriteDatabaseId,
                conf.appwriteCollectionId,
                slug,   // ✅ safe because slug transform ensures uniqueness
                {
                    title,
                    content,
                    featuredImage,
                    status,
                    userId,
                    slug,  // still store slug as field for queries
                }
            );
        } catch (error) {
